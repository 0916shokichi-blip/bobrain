...
MyBrain MCP is in early prototype phase. Issues, bug reports, and PRs are welcome.
...
```bash
uv run mybrain index ./your-notes -n notes
uv run mybrain search "some query" -k 5 --ns notes
uv run mybrain serve     # stdio MCP server
```
...
- Follow the patterns in `src/mybrain_mcp/` (small modules, pure functions where practical, dataclasses for records)
...
- A minimal repro: a few `.md` files plus the `mybrain` command(s) you ran
